...
public class OptimizerMain {
	...
	public static void main(String[] args) throws IOException, BiffException,
		...
		// Input Area Starts
		WritableWorkbook lineup = Workbook.createWorkbook(new File ("10-31 main top.xls"));
		shh = lineup.createSheet("sheet1", 0);

		Sheet sh = wb.getSheet("10-31 main");
		...
		for (int r = 1; r < sh.getRows(); r++) {
			...
			Player player = new Player(name, position, salary, team, value, score);
								...
								for (int pf1 = 0; pf1 < pf.size() - 1; pf1++)
									for (int pf2 = pf1 + 1; pf2 < pf.size(); pf2++)
										for (int c1 = 0; c1 < c.size(); c1++) {
											...
											if (salarySum>=59000 && salarySum<=60000 && scoreSum >=373 && otherConditions == true) {
												for (Player ele : roster) {
												...
												//System.out.println(count++ + "  " + roster.get(0).name + "   " + roster.get(1).name + "   " + roster.get(2).name);
//												if (count % 500 == 0){
//													System.out.print("continue enter 1: ");
//													if ( in.nextInt() != 1){
//														lineup.write();
//														lineup.close();
//														wb.close();
