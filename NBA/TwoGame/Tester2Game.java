...
public class Tester2Game {
	/*
	 * Tester should be exactly the same as Generator except that Tester includes the score column
	 * We use Tester after the game is over to observe
	 */
	...
	public static void main(String[] args) throws IOException, BiffException,
		...
		// Input Area Starts
		WritableWorkbook lineup = Workbook.createWorkbook(new File ("5-15 w scores.xls"));   // the excel that has scores
		shh = lineup.createSheet("sheet1", 0);

		Sheet sh = wb.getSheet("5-15");
		
		double min = -4.26;  // minimum consSum can be created from today's roster
		double max = 431;  // maximum valueSum can be created from today's roster
		
		double higherThan = 46;   // projected value
		double lowerThan = -0.6;  // consistency 
		//Input Area Ends
									...
									for (int pf2 = pf1 + 1; pf2 < pf.size(); pf2++)
										for (int c1 = 0; c1 < c.size(); c1++) {
											...
											for (Player ele : roster) {
												...
												// if that team already in the map, add 1; if not, put that team and set the value to 1
											...
											// FanDuel limits at most 4 players from the same team
...
//											findMaxMin();
	...
	public static void findMaxMin() throws RowsExceededException, WriteException {
		if ((salarySum <= 60000 && consSum <= -4 && otherConditions == true)
				|| (salarySum <= 60000 && valueSum >= 420 && otherConditions == true)) {
			for (Player ele : roster) {
