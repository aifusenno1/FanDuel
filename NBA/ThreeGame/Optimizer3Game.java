...
public class Optimizer3Game {
	...
	public static void main(String[] args) throws IOException, BiffException,
		...
		// Input Area Starts
		WritableWorkbook lineup = Workbook.createWorkbook(new File ("10-29 3-game top.xls"));
		shh = lineup.createSheet("sheet1", 0);

		Sheet sh = wb.getSheet("10-29 3-game");
		...
		shh.addCell(new Label(15,0,"Same Team"));
		shh.addCell(new Label(16,0,"Start"));
		shh.addCell(new Label(17,0,"Score"));
								...
								for (int pf1 = 0; pf1 < pf.size() - 1; pf1++)
									for (int pf2 = pf1 + 1; pf2 < pf.size(); pf2++)
										for (int c1 = 0; c1 < c.size(); c1++) {
											...
											if (salarySum>=58000 && salarySum<=60000 && scoreSum >=292 && otherConditions == true) {
												for (Player ele : roster) {
